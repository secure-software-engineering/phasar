...
AnalysisController::AnalysisController(ProjectIRDB &&IRDB,
    ...
    for (DataFlowAnalysisType analysis : Analyses) {
      ...
      switch (analysis) {
        case DataFlowAnalysisType::IFDS_TaintAnalysis: {
          ...
          cout << taintanalysisproblem.NtoString(
                      getNthInstruction(ICFG.getMethod("main"), 1))
               << endl;
          cout << taintanalysisproblem.getSolverConfiguration() << endl;
        ...
        case DataFlowAnalysisType::Plugin: {
          ...
          AnalysisPluginController(AnalysisPlugins, ICFG, EntryPoints);
          break;
        }
  ...
  // Perform module-wise (MW) analysis
    ...
    for (DataFlowAnalysisType analysis : Analyses) {
        ...
        switch (analysis) {
          case DataFlowAnalysisType::IFDS_TaintAnalysis: {
            ...
            LLVMMWAIFDSSolver<const llvm::Value *, LLVMBasedICFG &>
                llvmtaintsolver(taintanalysisproblem, SummaryGenerationStrategy::always_all, false);
      ...
      auto N = IRDB.getModuleDefiningFunction("_Z2idi");
      BOOST_LOG_SEV(lg, INFO) << "Combining module-wise icfgs";
      LLVMBasedICFG I = MWICFGs.at(M);
      I.printAsDot("main.dot");
      LLVMBasedICFG J = MWICFGs.at(N);
      J.printAsDot("id.dot");
      for (auto &entry : MWICFGs) {
        if (entry.first != M) {
