...
set<const llvm::Type*> LLVMStructTypeHierarchy::getTransitivelyReachableTypes(
    const llvm::Type* T) {
  ...
  for (tie(ei, ei_end) = boost::out_edges(type_vertex_map[T], tc); ei != ei_end;
       ++ei) {
...
vector<const llvm::Function*> LLVMStructTypeHierarchy::constructVTable(
    const llvm::Type* T, const llvm::Module* M) {
  vector<const llvm::Function*> VTable;
  for (auto& F : M->functions()) {
    if (F.getArgumentList().size() > 0) {
      if (F.getArgumentList().begin()->getType()->isPointerTy() &&
          F.getArgumentList().begin()->getType()->getPointerElementType() ==
              T &&
          F.hasAddressTaken()) {
        VTable.push_back(&F);
      }
...
const llvm::Function* LLVMStructTypeHierarchy::getFunctionFromVirtualCallSite(
  ...
  cout << "index: "
       ...
       << endl;
  return vtable[llvm::dyn_cast<llvm::ConstantInt>(gep->getOperand(1))
                    ->getZExtValue()];
}

bool LLVMStructTypeHierarchy::containsSubType(const llvm::Type* T,
                                              const llvm::Type* ST) {
  for (auto type : T->subtypes()) {
    for (unsigned i = 0; i < type->getNumContainedTypes(); ++i) {
      if (ST->getPointerElementType() == type->getContainedType(i)) return true;
    }
  }
  return false;
}

bool LLVMStructTypeHierarchy::hasSuperType(const llvm::Type* ST,
                                           const llvm::Type* T) {
  cout << "operation not supported, yet" << endl;
  return false;
}

bool LLVMStructTypeHierarchy::hasSubType(const llvm::Type* ST,
                                         const llvm::Type* T) {
  set<vertex_t> result;
  reachability_dfs_visitor vis(result);
  std::vector<boost::default_color_type> color_map(boost::num_vertices(g));
  boost::depth_first_visit(
      g, type_vertex_map[ST], vis,
...
ostream& operator<<(ostream& os, const LLVMStructTypeHierarchy& ch) {
  ...
  boost::print_graph(
      ch.g, boost::get(&LLVMStructTypeHierarchy::VertexProperties::name, ch.g));
  return os;
}
