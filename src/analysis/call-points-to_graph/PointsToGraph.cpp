...
PointsToGraph::PointsToGraph(llvm::AAResults& AA, const llvm::Function* Fu) : F(Fu)
{
	...
	llvm::Function* F = const_cast<llvm::Function*>(Fu);
	...
	const llvm::DataLayout &DL = M->getDataLayout();
		...
		for (llvm::inst_iterator I = inst_begin(F), E = inst_end(F); I != E;
			...
			if (auto CS = llvm::CallSite(&Inst)) {
				...
				for (llvm::Instruction::op_iterator OI = Inst.op_begin(), OE =
...
//				<< " pointers, " << CallSites.size() << " call sites\n";
		...
		for (llvm::SetVector<llvm::Value *>::iterator I1 = Pointers.begin(), E =
			...
			for (llvm::SetVector<llvm::Value *>::iterator I2 = Pointers.begin();
				...
				llvm::Type *I2ElTy = llvm::cast<llvm::PointerType>(
						(*I2)->getType())->getElementType();
				if (I2ElTy->isSized())
				...
				case llvm::PartialAlias:
					//PrintResults("PartialAlias", PrintPartialAlias, *I1, *I2, F->getParent());
...
{
	return V->getType()->isPointerTy() && !llvm::isa<llvm::ConstantPointerNull>(V);
}
...
set<const llvm::Value*> PointsToGraph::aliasWithFormalParameter(const llvm::Function* F, const llvm::Value* V)
{
	...
	boost::depth_first_visit(ptg, value_vertex_map[V], vis,
			boost::make_iterator_property_map(color_map.begin(), boost::get(boost::vertex_index, ptg), color_map[0]));
...
ostream& operator<<(ostream& os, const PointsToGraph& ptg)
{
	os << "PointsToGraph for " << ptg.F->getName().str() << "\n";
	boost::print_graph(ptg.ptg, boost::get(&PointsToGraph::VertexProperties::ir, ptg.ptg));
	return os;
}

void PointsToGraph::merge_graphs(PointsToGraph& g, const llvm::Value* v_in_g, const PointsToGraph& h, const llvm::Value* u_in_h)
{
...
//    IsoMap mapV = boost::make_iterator_property_map(orig2copy_data.begin(), get(boost::vertex_index, h.ptg));
//    boost::copy_graph(g.ptg, h.ptg, boost::orig_to_copy(mapV) ); //means g1 += g2
//
