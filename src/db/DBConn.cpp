...
{
	const static string database_initialization_command = "CREATE TABLE IF NOT EXISTS IR("
...
{
	static string query = "SELECT EXISTS(SELECT 1 FROM IR WHERE NAME='"+mod_name+"' LIMIT 1);";
	ResultSet result = execute(query);
	return result.data[0][0] == "1";
}

bool DBConn::insertFrontEndIRModule(const llvm::Module* module)
{
	...
	static string query = "INSERT INTO IR (MODULE_ID,"
										  "FRONTEND_IR_HASH,"
										  "FRONTEND_IR) "
										  "VALUES (?,?,?);";
	...
	// use SQLITE_TRANSIENT when SQLite should copy the string (e.g. when string is destroyed before query is executed)
	// use SQLITE_STATIC when the strings lifetime exceeds the point in time where the query is executed
	last_retcode = sqlite3_bind_text(statement, 1, module->getModuleIdentifier().c_str(), module->getModuleIdentifier().size(), SQLITE_STATIC);
	// compute hash of front-end IR
	string ir_hash = to_string(hash<string>()(ir_buffer));
	last_retcode = sqlite3_bind_text(statement, 2, ir_hash.c_str(), ir_hash.size(), SQLITE_STATIC);
	last_retcode = sqlite3_bind_blob(statement, 3, ir_buffer.data(), ir_buffer.size(), SQLITE_TRANSIENT);
...
unique_ptr<llvm::Module> DBConn::getFrontEndIRByName(string module_id, llvm::LLVMContext& Context)
{
...
//	string module_id;
	...
	last_retcode = sqlite3_prepare_v2(db, query.c_str(), -1, &statement, NULL);
	last_retcode = sqlite3_bind_text(statement, 1, module_id.c_str(), module_id.size(), SQLITE_STATIC);
	...
	memcpy(const_cast<char*>(ir_buffer.data()), sqlite3_column_blob(statement, 0), nbytes);
...
unique_ptr<llvm::Module> DBConn::getOptimizedIRByName(string mod_name)
{
	return nullptr;
}

size_t DBConn::getSourceHashByName(string mod_name)
{
	return 0;
}

size_t DBConn::getFrontEndIRHashByName(string mod_name)
{
	return 0;
}

set<string> DBConn::getAllIRModuleNames()
{
	return {};
}