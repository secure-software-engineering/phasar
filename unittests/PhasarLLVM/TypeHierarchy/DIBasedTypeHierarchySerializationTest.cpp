...
TEST(DBTHTest, BasicTHReconstruction_1) {
  ...
  DBTH.printAsJson(StringStream);

  auto DebugTest = DBTH.getAsJson();

  // deserialize data into CallGraphData object
  psr::DIBasedTypeHierarchy DeserializedTypeHierarchy(
      &IRDB, psr::DIBasedTypeHierarchyData::loadJsonString(Ser));

  compareResults(DBTH, DeserializedTypeHierarchy);
}
