...
TEST(LLVMBasedCFGTest, BranchTargetTest) {
  LLVMBasedCFG Cfg;
  LLVMProjectIRDB IRDB({"llvm_test_code/control_flow/switch.ll"});
  const auto *F = IRDB.getFunctionDefinition("main");
...
TEST(LLVMBasedCFGTest, HandleFieldLoadsArray) {
  LLVMBasedCFG Cfg;
  LLVMProjectIRDB IRDB({"llvm_test_code/fields/array_1.ll"});
  const auto *F = IRDB.getFunctionDefinition("main");
...
TEST(LLVMBasedCFGTest, HandleFieldLoadsField) {
  LLVMBasedCFG Cfg;
  LLVMProjectIRDB IRDB({"llvm_test_code/fields/field_1.ll"});
  const auto *F = IRDB.getFunctionDefinition("main");
  const auto *Inst = getNthInstruction(F, 1);
  ASSERT_FALSE(Cfg.isFieldLoad(Inst));
...
PHASAR_SKIP_TEST(TEST(LLVMBasedCFGTest, HandlesCppStandardType) {
  ...
  LLVMProjectIRDB IRDB({"llvm_test_code/name_mangling/special_members_2.ll"});
...
TEST(LLVMBasedCFGTest, HandlesCppUserDefinedType) {
  LLVMProjectIRDB IRDB({"llvm_test_code/name_mangling/special_members_1.ll"});
...
TEST(LLVMBasedCFGTest, HandlesCppNonStandardFunctions) {
  LLVMProjectIRDB IRDB({"llvm_test_code/name_mangling/special_members_3.ll"});
...
TEST(LLVMBasedCFGTest, HandleFunctionsContainingCodesInName) {
  LLVMProjectIRDB IRDB({"llvm_test_code/name_mangling/special_members_4.ll"});
...
TEST(LLVMBasedCFGTest, IgnoreSingleDbgInstructionsInSuccessors) {
  ...
  LLVMProjectIRDB IRDB1(
      {"llvm_test_code/control_flow/ignore_dbg_insts_1.dbg.ll"});
...
TEST(LLVMBasedCFGTest, IgnoreMultiSubsequentDbgInstructionsInSuccessors) {
  LLVMBasedCFG Cfg;
  LLVMProjectIRDB IRDB1(
      {"llvm_test_code/control_flow/ignore_dbg_insts_4.dbg.ll"});
  const auto *F = IRDB1.getFunctionDefinition("main");
