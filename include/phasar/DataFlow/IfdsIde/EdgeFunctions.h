...
public:
  ...
  ///
  /// Describes a value computation problem along a normal (non-call,
  /// non-return) intra-procedural exploded supergraph edge. A normal edge
  /// function implementation is queried for each edge that has been generated
  /// by appling the flow function returned by
  /// FlowFunctions::getNormalFlowFunction(). The supergraph edge whose
  /// computation is requested is defined by the supergraph nodes CurrNode and
  /// SuccNode.
  ///
  ...
  /// function is queried that describes a value computation. This results in
  ...
  /// flow function that has been returned by
  /// FlowFunctions::getCallFlowFunction. The supergraph edge whose computation
  /// is requested is defined by the supergraph nodes SrcNode and DestNode.
  ///
  /// The concrete implementation of an edge function e is depending on the
  /// analysis problem. In the following, we present a brief, contrived example:
  ///
  ...
  /// function implementation is queried for each edge that has been generated
  /// by applying the flow function that has been returned by
...
template <typename T>
concept EdgeFunctionFactory = requires(
    T &EFF, typename T::n_t Curr, typename T::n_t Succ,
    typename T::d_t CurrNode, typename T::d_t SuccNode,
    typename T::n_t ExitInst, typename T::n_t CallSite, typename T::n_t RetSite,
    typename T::f_t CalleeFun, llvm::ArrayRef<typename T::f_t> Callees) {
  ...
  /// \endcode
  {
    EFF.getNormalEdgeFunction(Curr, CurrNode, Succ, SuccNode)
  } -> std::convertible_to<typename T::EdgeFunctionType>;
  ...
  /// \endcode
  {
    EFF.getCallEdgeFunction(CallSite, CurrNode, CalleeFun, SuccNode)
  } -> std::convertible_to<typename T::EdgeFunctionType>;
  ...
  /// \endcode
  {
    EFF.getReturnEdgeFunction(CallSite, CalleeFun, ExitInst, CurrNode, RetSite,
                              SuccNode)
  } -> std::convertible_to<typename T::EdgeFunctionType>;
  ...
  ///
  {
    EFF.getCallToRetEdgeFunction(CallSite, CurrNode, RetSite, SuccNode, Callees)
  } -> std::convertible_to<typename T::EdgeFunctionType>;
  ...
  ///
  {
    EFF.getSummaryEdgeFunction(CallSite, CurrNode, RetSite, SuccNode)
  } -> std::convertible_to<typename T::EdgeFunctionType>;
};
