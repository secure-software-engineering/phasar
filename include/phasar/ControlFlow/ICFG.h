...
concept ICFG = CFG<T> && requires(const T &ICF, llvm::StringRef Name,
                                  typename std::remove_cvref_t<T>::n_t Inst,
  ...
  {
    ICF.getAllFunctions()
  } -> is_iterable_over_v<typename std::remove_cvref_t<T>::f_t>;
  // TODO: Should not be duplicated with ProjectIRDB
  {
    ICF.getFunction(Name)
  } -> std::convertible_to<Nullable<typename std::remove_cvref_t<T>::f_t>>;
  ...
  {
    ICF.getCalleesOfCallAt(Inst)
  } -> psr::is_iterable_over_v<typename std::remove_cvref_t<T>::f_t>;
...
concept ICFGOf =
    ICFG<T> && std::same_as<N, typename std::remove_cvref_t<T>::n_t> &&
