...
public:
    ...
    psr::MonoSet<D> getResult(psr::MonoSet<D> &In, const N src){
        ...
        if(contextmap.find(std::make_pair<psr::MonoSet<D>,N>(In,src))==contextmap.end()){
            ...
            return psr::MonoSet<D>(contextmap.find(std::make_pair<psr::MonoSet<D>,N>(In,src))->second);
        }
    } 
...
public:
    ...
    friend bool operator==(const ContextBase &lhs,
                            const ContextBase &rhs) {
