...
protected:
  ...
  Table<N, D, std::map<D, std::shared_ptr<EdgeFunction<L>>>> nonEmptyReverseLookup;
  // std::mapping from source value and target node to a list of all target values
  // and associated functions
  // where the list is implemented as a std::mapping from the source value to the
  // function
  // we exclude empty default functions
  Table<D, N, std::map<D, std::shared_ptr<EdgeFunction<L>>>> nonEmptyForwardLookup;
  // a std::mapping from target node to a list of triples consisting of source value,
...
public:
   ...
   */
  std::map<D, std::shared_ptr<EdgeFunction<L>>> reverseLookup(N target, D targetVal) {
   ...
   */
  std::map<D, std::shared_ptr<EdgeFunction<L>>> forwardLookup(D sourceVal, N target) {
  ...
  void printNonEmptyReverseLookup() {
    ...
    std::cout << "Table<N, D, std::map<D, std::shared_ptr<EdgeFunction<L>>>>" << std::endl;
  ...
  void printNonEmptyForwardLookup() {
    ...
    std::cout << "Table<D, N, std::map<D, std::shared_ptr<EdgeFunction<L>>>>" << std::endl;
  ...
  void printNonEmptyLookupByTargetNode() {
    ...
    std::cout << "unordered_std::map<N, Table<D, D, std::shared_ptr<EdgeFunction<L>>>>"
         << std::endl;
