...
private:
  ...
  const IDETabulationProblem<N, D, M, T, V, L, I> &problem;
...
public:
  JumpFunctions(EdgeFunction<L> *allTop,
                const IDETabulationProblem<N, D, M, T, V, L, I> &p)
   ...
   */
  std::unordered_map<D, EdgeFunction<L> *> reverseLookup(N target,
                                                         D targetVal) {
    if (!nonEmptyReverseLookup.contains(target, targetVal))
      return std::unordered_map<D, EdgeFunction<L> *>{};
    else
   ...
   */
  std::unordered_map<D, EdgeFunction<L> *> forwardLookup(D sourceVal,
                                                         N target) {
