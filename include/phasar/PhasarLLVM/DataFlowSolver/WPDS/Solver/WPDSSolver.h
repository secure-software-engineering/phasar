...
class WPDSSolver : public IDESolver<N, D, M, T, V, L, I> {
private:
  WPDSProblem<N, D, M, T, V, L, I> &Problem;
...
public:
  WPDSSolver(WPDSProblem<N, D, M, T, V, L, I> &Problem)
      : IDESolver<N, D, M, T, V, L, I>(Problem), Problem(Problem),
  ...
  void solve() override {
    auto &lg = lg::get();
    // Construct the PDS
    IDESolver<N, D, M, T, V, L, I>::submitInitalSeeds();
    ...
    if (WPDSSearchDirection::FORWARD == SolverConf.searchDirection) {
      ...
      auto retnode =
          wali::getKey(&IDESolver<N, D, M, T, V, L, I>::ICF->getFunction("main")
                            ->back()
                            .back());
  ...
  void processNormalFlow(PathEdge<N, D> edge) override {
    ...
    for (auto m : successorInst) {
      ...
      std::set<D> res =
          IDESolver<N, D, M, T, V, L, I>::computeNormalFlowFunction(
              flowFunction, d1, d2);
      ...
      for (D d3 : res) {
        ...
        LOG_IF_ENABLE(BOOST_LOG_SEV(lg, DEBUG)
                      ...
                      << Problem.NtoString(n) << " --> "
                      << Problem.DtoString(d3) << " | " << Problem.DtoString(m)
                      << ", " << *wptr << ")");
        ...
        IDESolver<N, D, M, T, V, L, I>::propagate(d1, m, d3, fprime, nullptr,
                                                  false);
      }
    }
  }
  ...
  void processCall(PathEdge<N, D> edge) override {
    ...
    std::set<M> callees =
        IDESolver<N, D, M, T, V, L, I>::ICF->getCalleesOfCallAt(n);
    ...
    for (M sCalledProcN : callees) { // still line 14
      ...
      if (specialSum) {
        ...
        for (N returnSiteN : returnSiteNs) {
          std::set<D> res =
              IDESolver<N, D, M, T, V, L, I>::computeSummaryFlowFunction(
                  specialSum, d1, d2);
          ...
          for (D d3 : res) {
            ...
            LOG_IF_ENABLE(BOOST_LOG_SEV(lg, DEBUG)
                          ...
                          << f->str());
            LOG_IF_ENABLE(BOOST_LOG_SEV(lg, DEBUG) << ' ');
            IDESolver<N, D, M, T, V, L, I>::propagate(
        ...
        INC_COUNTER("FF Queries", 1, PAMM_SEVERITY_LEVEL::Full);
        std::set<D> res =
            IDESolver<N, D, M, T, V, L, I>::computeCallFlowFunction(function,
                                                                    d1, d2);
        ADD_TO_HISTOGRAM("Data-flow facts", res.size(), 1,
                         PAMM_SEVERITY_LEVEL::Full);
        // for each callee's start point(s)
        std::set<N> startPointsOf =
            IDESolver<N, D, M, T, V, L, I>::ICF->getStartPointsOf(sCalledProcN);
        ...
        for (N sP : startPointsOf) {
          IDESolver<N, D, M, T, V, L, I>::saveEdges(n, sP, d2, res, true);
            ...
            for (typename Table<N, D, std::shared_ptr<EdgeFunction<L>>>::Cell
              ...
              for (N retSiteN : returnSiteNs) {
                ...
                std::set<D> returnedFacts =
                    IDESolver<N, D, M, T, V, L, I>::computeReturnFlowFunction(
                        retFunction, d3, d4, n, std::set<D>{d2});
                ADD_TO_HISTOGRAM("Data-flow facts", returnedFacts.size(), 1,
                                 PAMM_SEVERITY_LEVEL::Full);
                IDESolver<N, D, M, T, V, L, I>::saveEdges(eP, retSiteN, d4,
                                                          returnedFacts, true);
                ...
                for (D d5 : returnedFacts) {
                  ...
                  std::set<N> exitPointsN =
                      IDESolver<N, D, M, T, V, L, I>::ICF->getExitPointsOf(
                          IDESolver<N, D, M, T, V, L, I>::ICF->getFunctionOf(
                              sP));
      ...
      for (N returnSiteN : returnSiteNs) {
        ...
        std::set<D> returnFacts =
            IDESolver<N, D, M, T, V, L, I>::computeCallToReturnFlowFunction(
  ...
  void processExit(PathEdge<N, D> edge) override {
    ...
    std::set<N> startPointsOf =
        IDESolver<N, D, M, T, V, L, I>::ICF->getStartPointsOf(
            methodThatNeedsSummary);
    std::map<N, std::set<D>> inc;
    for (N sP : startPointsOf) {
      // line 21.1 of Naeem/Lhotak/Rodriguez
      // register end-summary
      IDESolver<N, D, M, T, V, L, I>::addEndSummary(sP, d1, n, d2, f);
      for (auto entry : IDESolver<N, D, M, T, V, L, I>::incoming(d1, sP)) {
        inc[entry.first] = std::set<D>{entry.second};
      }
    }
    IDESolver<N, D, M, T, V, L, I>::printEndSummaryTab();
    IDESolver<N, D, M, T, V, L, I>::printIncomingTab();
    ...
    if (IDESolver<N, D, M, T, V, L, I>::SolverConfig.followReturnsPastSeeds &&
        inc.empty() &&
        IDESolver<N, D, M, T, V, L, I>::ideTabulationProblem.isZeroValue(d1)) {
      std::set<N> callers = IDESolver<N, D, M, T, V, L, I>::ICF->getCallersOf(
          methodThatNeedsSummary);
      for (N c : callers) {
        for (N retSiteC :
             IDESolver<N, D, M, T, V, L, I>::ICF->getReturnSitesOfCallAt(c)) {
          ...
          std::set<D> targets =
              IDESolver<N, D, M, T, V, L, I>::computeReturnFlowFunction(
                  retFunction, d1, d2, c,
                  std::set<D>{IDESolver<N, D, M, T, V, L, I>::zeroValue});
          ADD_TO_HISTOGRAM("Data-flow facts", targets.size(), 1,
                           PAMM_SEVERITY_LEVEL::Full);
          IDESolver<N, D, M, T, V, L, I>::saveEdges(n, retSiteC, d2, targets,
                                                    true);
          for (D d5 : targets) {
            ...
            LOG_IF_ENABLE(BOOST_LOG_SEV(lg, DEBUG)
                          << "Compose: " << f5->str() << " * " << f->str());
            LOG_IF_ENABLE(BOOST_LOG_SEV(lg, DEBUG) << ' ');
            IDESolver<N, D, M, T, V, L, I>::propagteUnbalancedReturnFlow(
                retSiteC, d5, f->composeWith(f5), c);
            // register for value processing (2nd IDE phase)
            IDESolver<N, D, M, T, V, L, I>::unbalancedRetSites.insert(retSiteC);
          }
        }
      }
  ...
  void doForwardSearch(wali::wfa::WFA &Answer) {
    ...
    // <ZeroPDSState, entry>
    for (auto seed : IDESolver<N, D, M, T, V, L, I>::initialSeeds) {
  ...
  void doBackwardSearch(wali::Key node, wali::wfa::WFA &Answer) {
    ...
    PDS->for_each(syms);
    auto alloca1 = &IDESolver<N, D, M, T, V, L, I>::ICF->getFunction("main")
                        ->front()
                        .front();
