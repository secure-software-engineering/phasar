...
def usage():
    ...
    print("                            Form: accesstype[private|public|protected]:name:attributetype")
    print("                                  list seperated with comma")
    print("                                  put =value behind name for standard value")
    print("-f [--functions] <input>  : list of member functions that should be generated")
    print("                            Form: accesstype[private|public|protected]:name:returntype:[parameterlist]:[modifier list pre function]")
    print("                                  :[modifier list post function]:[template parameters]")
    ...
    print("-t [--template] <input>   : list of class template parameters; comma seperated")
...
def generateImplementationFiles():
    ...
    if "debug" in globals():
        print(bcolors.OKGREEN+"STOP:Generating implementation file..."+bcolors.ENDC)
...
def main(argv):
    ...
    for opt,arg in opts:
        if opt in ("-?", "--help"):
            ...
            if (clang!= "None" and clang!= "LLVM" and clang!= "Google" and clang!= "Chromium" and clang!= "Mozilla" and clang!= "WebKit"):
                ...
                print(bcolors.WARNING+"Ignoring clang-format flag..."+bcolors.ENDC)
                del clang
    ...
    if (platform.system()=="Linux" and "clang" in globals()):
        ...
        if clang=="None":     
            ...
            os.system("clang-format -style=file -i output/"+classname+".cpp")
        else:
            os.system("clang-format -style="+clang+" -i output/"+classname+".h")
            os.system("clang-format -style="+clang+" -i output/"+classname+".cpp")
        if "debug" in globals():
