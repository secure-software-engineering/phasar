...
IFDSFieldSensTaintAnalysis::getNormalFlowFunction(
  ...
  if (DataFlowUtils::isReturnValue(CurrentInst, SuccessorInst)) {
    return std::make_unique<ReturnInstFlowFunction>(SuccessorInst, traceStats,
                                                    getZeroValue());
  }

  if (llvm::isa<llvm::StoreInst>(CurrentInst)) {
    return std::make_unique<StoreInstFlowFunction>(CurrentInst, traceStats,
                                                   getZeroValue());
  }

  if (llvm::isa<llvm::BranchInst>(CurrentInst) ||
      llvm::isa<llvm::SwitchInst>(CurrentInst)) {
    return std::make_unique<BranchSwitchInstFlowFunction>(
        CurrentInst, traceStats, getZeroValue());
  }

  if (llvm::isa<llvm::GetElementPtrInst>(CurrentInst)) {
    return std::make_unique<GEPInstFlowFunction>(CurrentInst, traceStats,
                                                 getZeroValue());
  }

  if (llvm::isa<llvm::PHINode>(CurrentInst)) {
    return std::make_unique<PHINodeFlowFunction>(CurrentInst, traceStats,
                                                 getZeroValue());
  }

  if (DataFlowUtils::isCheckOperandsInst(CurrentInst)) {
    return std::make_unique<CheckOperandsFlowFunction>(CurrentInst, traceStats,
                                                       getZeroValue());
  }

  return std::make_unique<IdentityFlowFunction>(CurrentInst, traceStats,
                                                getZeroValue());
}
...
IFDSFieldSensTaintAnalysis::getCallFlowFunction(
  ...
  return std::make_unique<MapTaintedValuesToCallee>(
...
IFDSFieldSensTaintAnalysis::getRetFlowFunction(
  ...
  return std::make_unique<MapTaintedValuesToCaller>(
...
IFDSFieldSensTaintAnalysis::getCallToRetFlowFunction(
   ...
   * instruction fact.
   */
  return std::make_unique<CallToRetFlowFunction>(CallSite, traceStats,
                                                 getZeroValue());
}
...
IFDSFieldSensTaintAnalysis::getSummaryFlowFunction(
  ...
  if (!IsStaticCallSite) {
    return std::make_unique<IdentityFlowFunction>(CallStmt, traceStats,
                                                  getZeroValue());
  }
  ...
  if (llvm::isa<llvm::MemTransferInst>(CallStmt)) {
    return std::make_unique<MemTransferInstFlowFunction>(CallStmt, traceStats,
                                                         getZeroValue());
  }

  if (llvm::isa<llvm::MemSetInst>(CallStmt)) {
    return std::make_unique<MemSetInstFlowFunction>(CallStmt, traceStats,
                                                    getZeroValue());
  }

  if (llvm::isa<llvm::VAStartInst>(CallStmt)) {
    return std::make_unique<VAStartInstFlowFunction>(CallStmt, traceStats,
                                                     getZeroValue());
  }

  if (llvm::isa<llvm::VAEndInst>(CallStmt)) {
    return std::make_unique<VAEndInstFlowFunction>(CallStmt, traceStats,
                                                   getZeroValue());
  }
  ...
  if (taintConfig.isSource(DestFunName)) {
    return std::make_unique<GenerateFlowFunction>(CallStmt, traceStats,
                                                  getZeroValue());
  }
