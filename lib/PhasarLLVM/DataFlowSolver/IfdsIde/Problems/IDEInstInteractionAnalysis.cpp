...
void IDEInstInteractionAnalysis::registerFlowFactGenerator(
    std::function<std::set<IDEInstInteractionAnalysis::d_t>(
        IDEInstInteractionAnalysis::n_t)>
        FactGenerator) {
  FactGen = FactGenerator;
}
...
IDEInstInteractionAnalysis::getNormalFlowFunction(
  ...
  std::set<IDEInstInteractionAnalysis::d_t> UserFacts;
  if (FactGen) {
    UserFacts = FactGen(curr);
  }
  ...
  struct IIAFlowFunction : FlowFunction<IDEInstInteractionAnalysis::d_t> {
    ...
    IIAFlowFunction(IDEInstInteractionAnalysis &Problem,
                    IDEInstInteractionAnalysis::n_t Inst,
                    std::set<IDEInstInteractionAnalysis::d_t> &UserFacts)
        : Problem(Problem), Inst(Inst), UserFacts(UserFacts) {}
    ...
    computeTargets(IDEInstInteractionAnalysis::d_t src) override {
      ...
      if (Problem.isZeroValue(src)) {
        Facts.insert(UserFacts.begin(), UserFacts.end());
  ...
  return std::make_shared<IIAFlowFunction>(*this, curr, UserFacts);
}
