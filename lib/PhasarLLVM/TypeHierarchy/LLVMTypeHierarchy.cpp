...
bool LLVMTypeHierarchy::hasType(const llvm::StructType *Type) const {
  return type_vertex_map.count(Type);
}

bool LLVMTypeHierarchy::isSubType(const llvm::StructType *Type,
                                  const llvm::StructType *SubType) {
  auto ReachableTypes = getReachableSubTypes(Type);
  return ReachableTypes.count(SubType);
}

std::set<const llvm::StructType *>
LLVMTypeHierarchy::getReachableSubTypes(const llvm::StructType *Type) {
  return g[type_vertex_map[Type]].ReachableTypes;
}
...
bool LLVMTypeHierarchy::hasVFTable(const llvm::StructType *Type) const {
  return type_vtbl_map.count(Type);
}

LLVMVFTable *LLVMTypeHierarchy::getVFTable(const llvm::StructType *Type) const {
  // return type_vtbl_map[Type];
  return nullptr;
}

size_t LLVMTypeHierarchy::size() const { return boost::num_vertices(g); }
