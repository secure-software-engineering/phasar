...
struct CachedTypeGraph::reverse_type_propagation_dfs_visitor : public boost::default_dfs_visitor {
...
CachedTypeGraph::vertex_t CachedTypeGraph::addType(const llvm::StructType* new_type) {
...
void CachedTypeGraph::reverseTypePropagation(const llvm::StructType *base_struct) {
  auto name = psr::uniformTypeName(base_struct->getName().str());

  std::vector<boost::default_color_type> color_map(boost::num_vertices(g));

  auto reversed = boost::reverse_graph<CachedTypeGraph::graph_t, CachedTypeGraph::graph_t&>(g);
