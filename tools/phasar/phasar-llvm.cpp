...
int main(int argc, const char **argv) {
  ...
  try {
    ...
    // clang-format off
    Config.add_options()
			("function,F", boost::program_options::value<std::string>(), "Function under analysis (a mangled function name)")
    ...
    std::cerr << "Exception ocured while handling the command-line arguments!\n"
              << "Error: " << Err.what() << '\n';
  }
}
