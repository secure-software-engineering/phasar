...
RUN apt-get -qq -y install libmysqlcppconn-dev zlib1g-dev libncurses5-dev sqlite3 libsqlite3-dev bear python3 doxygen graphviz

# boost dependencies
RUN apt-get -qq -y install wget gcc g++
...
RUN cd /opt/framework/ && chmod -R 777 . && \
    ./misc/make_config.sh
